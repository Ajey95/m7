...
class DonationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // We need to handle donorName and address. 
    // Since Task model might not have them directly yet (we updated serialization but maybe not Task.fromJson fully?), 
    // we'll use safe access or placeholders. 
    // Ideally Task model should be updated to parse 'donor_name' and 'pickup_address' if the backend sends them.
    // For now we'll assume they might be missing or added to Task.dart later. 
    // If Task.dart doesn't have them, we can't access them here without updating Task.dart.
    // Let's assume for this specific file that we will stick to what Task has or use generic text.
    
    // We can now use the actual fields from the Task model
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    task.foodTypeString, // Helper we added to Task
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 12,
                      fontWeight: FontWeight.bold,
